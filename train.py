...
def clean_df():
    ...
    df = pd.read_csv("./Data/Gait_Data_Basic.csv")
    print(df)
    df = df.drop('Steps', 1)
    ...
    df["AudioOn"] = [
        1 if item == True else 0 for item in list(df["AudioOn"])
    ]
    ...
    target_column = "AudioOn"
    labels = np.array(df[target_column])
    feature_list = [i for i in list(df.columns) if i != "AudioOn"]
    df = df#.transpose()
    df.drop(["AudioOn"], axis=1, inplace=True)
...
def show_pred_real(test_features, test_labels, predictions):
    ...
    """
    combined_df = pd.DataFrame(test_features, columns=["Pace", "Candence"])
